...
export default async function MessagesPage() {
  ...
  const { data: messages } = await supabase
    .from("messages")
    .select("id, subject, body, read_at, created_at, sender:profiles!messages_sender_id_fkey(full_name)")
            ...
            return (
              <li key={m.id} className="flex items-start gap-3 p-4">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className={`truncate text-sm ${isUnread ? "font-medium text-slate-800" : "text-slate-600"}`}>
                    {m.subject}
                  </p>
